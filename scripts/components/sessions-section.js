...
document.addEventListener("DOMContentLoaded", function () {
  const sessionSortDropDownButton = document.getElementById(
    "sessions-sort-dropdown"
  );
  const sessionSortDropDownMenu = document.getElementById("sessions-sort-menu");
  const sessionSortSelectedType = document.getElementById(
  ...
  document.addEventListener("click", function (e) {
    if (
      !sessionSortDropDownButton.contains(e.target) &&
      !sessionSortDropDownMenu.contains(e.target)
    ) {
      sessionSortDropDownMenu.classList.add("hidden");
    }
  });
...
let sessionItem = Array.from(
  document.getElementsByClassName("session-tab-list-item")
);
let sessionItemHead = Array.from(
  document.getElementsByClassName("session-item-header")
);
let sessionItemBody = Array.from(
  document.getElementsByClassName("session-item-body")
);
...
sessionItemsContainer.addEventListener("click", (event) => {
  ...
  if (sessionItem) {
    ...
    const index = Array.from(sessionItem.parentNode.children).indexOf(
      sessionItem
    );
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document
    .querySelectorAll(".session-slider-container")
...
let sessionFileVisibilityBtn = document.getElementsByClassName(
  "session-file-visibility-btn"
);
let sessionFileVisibilityIcon =
  document.getElementsByClassName("visibility-icon");
let sessionFileVisibilityOffIcon = document.getElementsByClassName(
  "visibility-off-icon"
);
...
function isValidTimestamp(timestamp) {
  return (
    timestamp && timestamp.toDate && typeof timestamp.toDate === "function"
  );
}
...
function initializeTimeUpdates() {
  ...
  setInterval(() => {
    ...
    sessionHeaders.forEach((header) => {
      const createdSpan = header.querySelector(
        ".text-sm.text-silver-chalice-400.italic"
      );
      const timestamp = createdSpan.dataset.timestamp; // Add this data attribute

      if (timestamp) {
        createdSpan.textContent = `Created ${formatCreatedTime(
...
function createSessionElement(session) {
  ...
  sessionElement.className =
    "session-tab-list-item border-b border-silver-chalice-300 py-2 w-full h-auto flex flex-col";
                    ...
                    <div class="h-10 w-10 rounded bg-silver-chalice-200">
                        ${
                          session.preview_image
                            ? `<img src="${session.preview_image}" class="h-full w-full object-cover rounded" alt="Preview"/>`
                            : ""
                        }
                    </div>
                </div>
                <div class="flex-[2] h-full min-h-10 flex flex-col overflow-hidden">
                    <h4 class="text-lochmara-950 font-medium leading-none w-full truncate">${getSessionDisplayName(
                      session
                    )}</h4>
                    <div class="flex gap-x-2">
                        <span class="text-sm text-silver-chalice-400 italic">Created ${formatCreatedTime(
                          session.created_at
                        )}</span>
                        <span class="text-sm text-silver-chalice-400 italic">•</span>
                        <span class="text-sm text-silver-chalice-400 italic">${formatLongDuration(
                          session.expires_at
                        )}</span>
                    </div>
                </div>
                ...
                <div class="flex-[2] h-full min-h-10 flex flex-col justify-center gap-y-1">
                    <div class="session-usage-progress-bar-container w-[80%] h-2 rounded-full bg-lochmara-200">
                        <div class="session-usage-progress-bar h-full rounded-[1rem] bg-lochmara-600" 
                            style="width: ${
                              ((session.usage?.total_size || 0) /
                                ((session.limits.max_size / (1024 * 1024)) *
                                  1024 *
                                  1024)) *
                              100
                            }%"></div>
...
async function displaySessionFiles(sessionId) {
  ...
  const sessionBody = sessionElement.querySelector(".session-item-body");
    ...
    const fileData = filesSnap.docs.map(doc => ({
        id: doc.id,
        ...doc.data()
    }));
    ...
    if (!controlsWrapper) {
        ...
        const thresholdControlsWrapper = document.createElement("div");
        ...
        sessionBody.insertBefore(controlsWrapper, sessionBody.secondChild);
    }
    ...
    fileData.forEach(file => {
        ...
        requestAnimationFrame(() => {
            fileElement.style.cssText = `
                background-image: url('${file.storage_url}');
        ...
        fileElement.querySelector(".session-file-delete-btn").addEventListener("click", async (e) => {
            ...
            try {
                if (await handleFileDelete(sessionId, file.id)) {
                    fileElement.remove();
  ...
  nameElement.addEventListener("click", (e) => {
    ...
    const saveNewName = async (newName) => {
      if (!validateName(newName)) {
        alert(
          "Name must be 1-50 characters and contain only letters, numbers, spaces, dash or underscore"
        );
        input.focus();
        return false;
      }
